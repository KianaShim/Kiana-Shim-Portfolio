...
CREATE PROCEDURE sp_createSpecies(
    IN p_speciesName varchar(50),
    IN p_pokedexEntry INT,
    IN p_pokemonType1 varchar(50),
    IN p_pokemonType2 varchar(50)
)
COMMENT 'Creates a new pokemon species'
BEGIN
    ...
    (p_speciesName, p_pokedexEntry, p_pokemonType1, p_pokemonType2);
    COMMIT;
END //

DELIMITER ;

DROP PROCEDURE IF EXISTS sp_updateSpecies;
DELIMITER //
...
COMMENT 'Updates a pokemon species'
BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        Rollback;
    END;
    START TRANSACTION;
    UPDATE PokemonSpecies SET pokedexEntry = p_pokedexEntry, pokemonType1 = p_pokemonType1, pokemonType2 = p_pokemonType2 WHERE speciesName = p_speciesName;
    COMMIT;
END //
...
DELIMITER //
CREATE PROCEDURE sp_deleteSpecies(IN p_speciesName VARCHAR(50))
BEGIN
    ...
    START TRANSACTION;
        DELETE FROM PokemonSpecies WHERE speciesName = p_speciesName;

        -- ROW_COUNT() returns the number of rows affected by the preceding statement.
        IF ROW_COUNT() = 0 THEN
            set error_message = CONCAT('No matching record found in bsg_people for species name: ', p_speciesName);
...
-- Reset and Load from DDL

DROP PROCEDURE IF EXISTS sp_loadTeamRocket;
DELIMITER //
CREATE PROCEDURE sp_loadTeamRocket()
BEGIN
